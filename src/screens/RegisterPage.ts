...
export type FieldCase = { label: string; value: string; expected: string; notes?: string };
export type NameCase = { label: string; value: string; shouldError: boolean; notes?: string };
...
export class RegisterPage extends BasePage {
  ...
  static readonly EMAIL_CASES: FieldCase[] = [
    ...
    { label: 'no @ or domain', value: 'plainaddress', expected: 'Enter a valid email' },
    { label: 'missing local part', value: '@missinglocal.com', expected: 'Enter a valid email' },
    { label: 'missing domain', value: 'missingdomain@', expected: 'Enter a valid email' },
    { label: 'missing TLD', value: 'missingtld@domain', expected: 'Enter a valid email' },
    { label: 'double @', value: 'two@@symbols.com', expected: 'Enter a valid email' },
    { label: 'multiple @', value: 'user@domain@domain.com', expected: 'Enter a valid email' },
    { label: 'space in local part', value: 'user name@domain.com', expected: 'Enter a valid email' },
    { label: 'space in domain', value: 'user@domain .com', expected: 'Enter a valid email' },
    { label: 'leading space', value: ' user@domain.com', expected: 'Enter a valid email' },
    { label: 'trailing space', value: 'user@domain.com ', expected: 'Enter a valid email' },
    { label: 'consecutive dots in local part', value: 'user..name@domain.com', expected: 'Enter a valid email' },
    { label: 'leading dot in local part', value: '.user@domain.com', expected: 'Enter a valid email' },
    { label: 'trailing dot before @', value: 'user.@domain.com', expected: 'Enter a valid email' },
    { label: 'domain starts with hyphen', value: 'user@-domain.com', expected: 'Enter a valid email' },
    {
    ...
    { label: 'consecutive dots in domain', value: 'user@domain..com', expected: 'Enter a valid email' },
    { label: 'domain starts with dot', value: 'user@.domain.com', expected: 'Enter a valid email' },
    { label: 'comma instead of dot', value: 'user@domain,com', expected: 'Enter a valid email' },
    { label: 'angle brackets / injection-shaped input', value: 'user<script>@domain.com', expected: 'Enter a valid email' },
    { label: 'emoji in local part', value: '😀@domain.com', expected: 'Enter a valid email' },
  ...
  static readonly NAME_CASES: NameCase[] = [
    ...
    { label: 'exactly 2 characters (minimum valid length)', value: 'Jo', shouldError: false },
    {
    ...
    },
    {
      ...
      value: 'John!',
      shouldError: true,
      notes: 'Same gap as the digit case — no character-type restriction on name fields.',
    },
    {
    ...
    { label: 'hyphenated name (e.g. "Mary-Jane")', value: 'Mary-Jane', shouldError: false },
    { label: "name with an apostrophe (e.g. \"O'Brien\")", value: "O'Brien", shouldError: false },
  ];

  static readonly PASSWORD_CASES: FieldCase[] = [
    { label: 'empty string', value: '', expected: 'At least 8 characters' },
    { label: '7 characters (one below minimum)', value: 'Pass12x', expected: 'At least 8 characters' },
    { label: 'no uppercase letter', value: 'password1', expected: 'Add one uppercase letter' },
    { label: 'no digit', value: 'NoDigitsHere', expected: 'Add one number' },
    { label: 'exactly 8 characters (minimum valid length)', value: 'Pass123x', expected: '' },
    {
    ...
    },
    {
      ...
      expected: '',
      notes: 'The client has no upper bound, while the backend caps at 128 chars (@Size(max=128)) — no client-side cap.',
    },
  ];

  private async fillValidDefaultsExcept(field: 'firstName' | 'lastName' | 'email' | 'password'): Promise<void> {
    if (field !== 'firstName') await this.enterFirstName('Test');
    if (field !== 'lastName') await this.enterLastName('User');
    if (field !== 'email') await this.enterEmail(`valid.${Date.now()}@example.com`);
    if (field !== 'password') await this.enterPassword('ValidPass123');
  }

  async checkFieldValidation(field: 'firstName' | 'lastName' | 'email' | 'password', value: string): Promise<string> {
